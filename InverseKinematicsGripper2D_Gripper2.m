function [Theta, JointCoord, AdditionalJointCoord] = InverseKinematicsGripper2D_Gripper2(L_act, L0, L16, CurrentLinks, PointM, PointN)
%% Function: InverseKinematicsGripper2D_Gripper2
% Summary: The InverseKinematicsGripper2D_Gripper2 function takes the 
%          inverse kinematics of the closed chain gripper structure (given 
%          the link lengths, kinematic constraints and location of the end 
%          effectors [the tip of the toes Points M and N], calculate the 
%          Theta angles of each of the links). Note: Theta of each of the 
%          links are measured from the positive x-axis. 
%
...
%
% INPUTS:
%    L_act: Determines the x position of Joints (Q)
%    L0: Determines the y position of Joints (I and J)
%    L16: Length of Link between Points (Q and D)
...
%
%    PointN: this represents the given x and y coordinates of Point N. The
...
r_IM = sqrt((x_M)^2 + (y_M - L0/2)^2);
theta11 = acos((r_IM^2 + L11^2 - (L12 + L14)^2)/(2*r_IM*L11)) + atan2(y_M - L0/2, x_M);
theta12 = theta11  +  acos((L11^2 + (L12 + L14)^2 - r_IM^2)/(2*L11*(L12+L14)))  -  pi;

%% Calculating theta8, theta9, given x_N and y_N
r_JN = sqrt((x_N)^2 + (y_N + L0/2)^2);
theta8 = atan2(y_N + L0/2, x_N)  -  acos((r_JN^2 + L8^2 - (L9+L15)^2)/(2*r_JN*L8));
...
x_G = (L11*cos(theta11) + (L12)*cos(theta12));
...
x_L = (L8*cos(theta8) + (L9)*cos(theta9));
...
%% Calculating x_D, y_D, given 

x_D = (L1 + L3*cos(theta3) + L4*cos(theta4));
y_D = (L2 + L3*sin(theta3) + L4*sin(theta4));

theta16 = asin(y_D/L16);

x_Q = x_D - L16*cos(theta16);
y_Q = 0;
...
% Point G  
P_G = [(L11*cos(theta11) + (L12)*cos(theta12));...
...
% Point L   
P_L = [(L8*cos(theta8) + (L9)*cos(theta9));...
