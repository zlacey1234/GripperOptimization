...
Dx=opti.variable(); opti.subject_to(0<Dx); opti.set_initial(Dx,120);

opti.subject_to(PointM_D(2)==Dx+offset_AD); opti.subject_to(PointN_D(2)==-Dx+offset_AD);
opti.subject_to(PointN_D(2)<PointM_D(2))
    % init 
opti.set_initial(PointM_D,[10,10]); opti.set_initial(PointN_D,[10,-10]);

 

%% inverse kinematics for passive adaptationyh
[Theta, JointCoord] = InverseKinematicsGripper2D(L_act, L0, CurrentLinks, PointM_D, PointN_D);
...
opti.subject_to(-L2 < D_y < L2);
 ...
 %% calculate weight
 min_ratio = 20; min_sf =2;
 w_a = weightSF((F_M(2)-F_N(2)),min_ratio,min_sf,5,1/20);
 opti.subject_to(min_sf*min_ratio < (F_M(2)-F_N(2)));
 
 %% calculate density
 PointM_a = JointCoord(1:2,13);
 PointN_a = JointCoord(1:2,14);
 rho_a = hold_pdf([88,PointM_a(2)-PointN_a(2)]);
...
opti.set_initial(L1,12);
opti.set_initial(L2,12);
opti.set_initial(L3,12);
opti.set_initial(L4,12);
opti.set_initial(L11,18);
opti.set_initial(L12,16);
opti.set_initial(L13,14);
opti.set_initial(L14,4);
