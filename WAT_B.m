...
By=opti.variable(); opti.subject_to(0<By); opti.set_initial(By,20);
...
opti.subject_to(PointN_B(2)<PointM_B(2)); opti.subject_to(PointN_B(1)<=PointM_B(1));
    % init 
opti.set_initial(PointM_B,[100,30]); opti.set_initial(PointN_B,[90,-20]);

 

%% inverse kinematics for passive adaptationyh
[Theta, JointCoord] = InverseKinematicsGripper2D(L_act, L0, CurrentLinks, PointM_B, PointN_B);
...
%opti.subject_to(-L2 < D_y < L2);
 ...
 w_B = weightSF((F_M(2)-F_N(2)),min_ratio,min_sf,5,1/20);
 opti.subject_to(min_sf*min_ratio < (F_M(2)-F_N(2)));
 
 %% calculate density
 PointM_B = JointCoord(1:2,13); PointN_B = JointCoord(1:2,14);
