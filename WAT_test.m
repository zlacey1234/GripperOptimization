...
opti.subject_to(0<PointN(1));
    ...
    % cube object y=+/-12
    Ax=opti.variable();
    opti.subject_to(0<Ax);
    opti.set_initial(Ax,2);
    offset = 0;

    opti.subject_to(PointM(2)==Ax+offset);
    opti.subject_to(PointN(2)==-Ax+offset);
    opti.subject_to(PointN(2)<PointM(2))
    % init 
    opti.set_initial(PointM,[10,10]);
    opti.set_initial(PointN,[10,-10]);
