...
Ay=opti.variable(); opti.subject_to(0<Ay); opti.set_initial(Ay,60*ampP);
...
opti.set_initial(PointM_A,[99,30]*ampP); opti.set_initial(PointN_A,[99,-30]*ampP);
...
Dy=opti.variable(); opti.subject_to(0<Dy); opti.set_initial(Dy,300*ampP);
...
% init for point M and N
opti.set_initial(PointM_D,[197,150]*ampP); opti.set_initial(PointN_D,[197,-150]*ampP);
...
By=opti.variable(); opti.subject_to(0<By); opti.set_initial(By,60*ampP);
...
opti.set_initial(PointM_B,[197.2,45]*ampP); opti.set_initial(PointN_B,[243,-5]*ampP);
...
Cy=opti.variable(); opti.subject_to(0<Cy); opti.set_initial(Cy,280*ampP);
...
opti.set_initial(PointM_C,[212,160]*ampP); opti.set_initial(PointN_C,[205,-120]*ampP);
...
[F_M_D, F_N_D] = StaticEquilibrium(F_actuator_total, Actuator_joint_num, CurrentLinks, Theta_D);
 ...
 %% calculate weight
 min_ratio = 10; min_sf =2; peak = 5; alpha = 1/10;
 % at edge A
...
opti.subject_to((PointM_A_calc(2)-PointN_A_calc(2)) < 5*(PointM_B_calc(2)-PointN_B_calc(2)));
%% Monte Carlo Sampling

A_d = JointCoord_A(1:2,4); B_d = JointCoord_B(1:2,4); C_d = JointCoord_C(1:2,4); D_d = JointCoord_D(1:2,4);
a1 = (A_d(2)-C_d(2))/(A_d(1)-C_d(1));
b1 = A_d(2) - a1*A_d(1);
a2 = (B_d(2)-D_d(2))/(B_d(1)-D_d(1));
b2 = B_d(2) - a2*B_d(1);
...
%     CDF = CDF + if_else(w_all(k)*rho_all(k)==w_all(k)*rho_all(k),w_all(k)*rho_all(k),0);
% end
...
%%
plot(opti.debug.value(debug_D(1,:)),opti.debug.value(debug_D(2,:)),'.')
hold on
plot(opti.debug.value(A_d(1)),opti.debug.value(A_d(2)),'*')
plot(opti.debug.value(B_d(1)),opti.debug.value(B_d(2)),'*')
plot(opti.debug.value(C_d(1)),opti.debug.value(C_d(2)),'*')
plot(opti.debug.value(D_d(1)),opti.debug.value(D_d(2)),'*')
legend
hold off

figure
PointM_debug = opti.debug.value(PointM_D);
PointN_debug = opti.debug.value(PointN_D);
[Theta_sol, JointCoord_debug] = InverseKinematicsGripper2D(L_act, opti.debug.value(L0), opti.debug.value(CurrentLinks), PointM_debug, PointN_debug);

DrawingGripper(JointCoord_debug,[10,0])
%%
